...
const BrowsingPageSearch = ({ searchTerm }) => {
  ...
  useEffect(() => {
    const getSongs = async () => {
      const { data } = await api.get(`/songs/name/${searchTerm}`);
      //TODO error handler
      setSongsData(data);
    };

    const getAlbums = async () => {
      const { data } = await api.get(`/albums/name/${searchTerm}`);
      //TODO error handler
      setAlbumsData(data);
    };

    const getArtists = async () => {
      const { data } = await api.get(`/artists/name/${searchTerm}`);
      //TODO error handler
      setArtistsData(data);
    };

    const getGenres = async () => {
      const { data } = await api.get(`/genres/name/${searchTerm}`);
      //TODO error handler
      setGenresData(data);
    };

    const getPlaylists = async () => {
      const { data } = await api.get(`/playlists/name/${searchTerm}`);
      //TODO error handler
