...
const SongModal = ({ openFlag, closeHandler, data, currentUser }) => {
  ...
  const handleToggleLike = () => {
    const toggleLike = async () => {
      const response = await api.put(`/likes/songs/${data.id}`);
  ...
  const handleDeleteClick = (commentId) => {
    const reloadComments = async () => {
      const commentsData = await api.get(`/comments/song/${data.id}`);
  ...
  useEffect(() => {
    const getComments = async () => {
      ...
      for (const commentId of data.comments) {
        const commentData = await api.get(`/comments/${commentId}`);
